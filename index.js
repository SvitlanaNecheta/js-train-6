...
const personLarge= {
name:person.name,//вико ристовуємо деструктурізацію на об'єкті person
age:person.age,
addres:{street: "123 Main St", city: "New York", country: "USA"}}//створюємо об'єкт address;
...
function copyObject(obj) {
 const newObject={...obj}; // Використовуємо синтаксис деструктурізації {...person} для створення нового об'єкта з тими ж властивостями
  return newObject;// Повертаємо новий об'єкт
}
...
function printKeysAndValues(obj) {
 for(const key in obj) {// Проходимося по всіх ключах об'єкту за допомогою циклу "for in"
...
let user = {
  name: "John",
  age: 25,
  introduce: function(){ return `My name is ${this.name} and I am ${this.age} years old.`}// Створюємо метод introduce, який за допомогою ключового слова this має повернути такий рядок
...
function changeRole(array, newRole) {
  ...
  for(const k of array){ // Ітеруємося по масиву об'єктів за допомогою циклу "for of"
  k.role=newRole;// Змінюємо роль кожного користувача на нове ім'я
 }
...
function printProductDetails(obj) {
  let {productName, price, manufacturer:{companyName, country}}=obj;// Використовуємо деструктуризацію для отримання значень productName, price i також значень companyName, country вкладеного об'єкту manufacturer
